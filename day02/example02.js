...
class TransactionBook {
  ...
  calcAmountForAccount(account) {
    return this.transactions.reduce((amount, transaction) => {
      ...
      if (transaction.recipient.name === account.name) {
        return amount + transaction.amount;
      }
      return amount;
    }, 0);
  ...
  findTransactionsForAccount(account) {
    return this.transactions.filter(
      transaction =>
        (transaction.sender && transaction.sender.name === account.name) ||
        transaction.recipient.name === account.name
    );
  }
}
...
class Account {
  constructor(name, bankAccount) {
    this.name = name;
    this.transactionBook = bankAccount;
    this.comission = 0;
  }
